...
resource "aws_security_group" "k8s_cluster_sg" {
  ...
  # ------------------------
  # SG → SG RULES (now referencing NEW SG)
  # ------------------------
...
resource "local_file" "ansible_inventory" {
  content = <<EOT
[master]
${module.k8s_master.public_ip} ansible_user=ubuntu ansible_ssh_private_key_file=./k8s-key.pem

[workers]
${module.k8s_worker.public_ip} ansible_user=ubuntu ansible_ssh_private_key_file=./k8s-key.pem
  EOT
